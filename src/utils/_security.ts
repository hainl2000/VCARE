...
import { ISODateRegex, ImageRegex, dateRegex } from 'src/constants/regex';
...
export function convertImage(target: any) {
  if (target === null) {
    return null;
  }
  if (typeof target === 'string' && ImageRegex.test(target)) {
    return process.env.URL_BUCKET + target;
  }
  if (typeof target === 'object') {
    if (Array.isArray(target)) {
      for (let i = 0; i < target.length; i++) {
